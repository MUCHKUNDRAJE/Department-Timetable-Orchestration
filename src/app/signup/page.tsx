...
import { Building2, Lock, User, Eye, EyeOff, ArrowRight, UserPlus, AlertCircle, CheckCircle2 } from 'lucide-react';
...
function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!/^[a-zA-Z0-9_-]+$/.test(username.trim())) {
      setLocalError('Username may only contain letters, numbers, hyphens, and underscores.');
      return;
    }
    if (!password) {
    ...
    try {
      ...
      toast.success('Account Created Successfully', `Welcome to Timetable Studio, @${username.trim()}!`);
      router.replace(from);
    } catch (err: any) {
    ...
    <div className="w-full max-w-md">
      ...
      <div className="bg-surface border border-border rounded-2xl shadow-xl shadow-primary/5 p-6 sm:p-8 backdrop-blur-md">
        <div className="flex items-center justify-between pb-5 mb-5 border-b border-border/80">
          <div>
            <h2 className="text-base font-bold text-foreground">Administrator Registration</h2>
            <p className="text-xs text-muted">Register an authorized account to manage timetables</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="e.g. Dr. Muchkundraje Thote"
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="e.g. admin or m_thote"
                className="w-full pl-10 pr-3.5 py-2.5 bg-surface-subtle border border-border rounded-xl text-xs text-foreground placeholder:text-muted/60 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
              />
            </div>
          </div>
          ...
          <div>
            <label className="block text-xs font-semibold text-foreground mb-1.5" htmlFor="password">
              Password <span className="text-primary">*</span> <span className="text-muted text-[10px]">(min. 6 characters)</span>
            </label>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-muted">
          ...
          {/* Confirm Password */}
          <div>
            <label className="block text-xs font-semibold text-foreground mb-1.5" htmlFor="confirmPassword">
    ...
    <div className="min-h-[calc(100vh-140px)] flex items-center justify-center py-10 px-4">
      <Suspense fallback={
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-xs font-medium text-muted">Loading sign up...</p>
        </div>
      }>
        <SignupForm />
      </Suspense>
